...
# Always one panel per library that returned data
n_panels  = len(db_groups)
 
# Dynamic figure height based on max terms across panels
max_rows   = max(len(g) for g in db_groups.values())
fig_height = max(6, max_rows * 0.55 + 2.5)
...
for n_genes in [1, 3, 5]:
    legend_ax.scatter(
        [], [], s=n_genes * DOT_SCALE,
        c="#aaaaaa", edgecolors="#262626", linewidths=0.45,
        label=str(n_genes),
    )
legend_ax.legend(
...
# Step 5: Write genes per displayed term to file
OUT_TXT = "results/psme1_kd_pathway_genes.txt"
print(f"\nWriting gene associations to {OUT_TXT}...")
 
with open(OUT_TXT, "w") as f:
    f.write("Pathway Enrichment — PSME1 KD vs Scramble — NCI-H441\n")
    f.write(f"{len(all_genes)} DEPs ({len(up_genes)} up, {len(down_genes)} down) | FDR < 0.05 | |log2FC| > 1\n")
    f.write("=" * 80 + "\n")
 
    for db_label in DB_ORDER:
        if db_label not in db_groups:
            continue
        grp = db_groups[db_label].sort_values("Adjusted P-value").reset_index(drop=True)
        f.write(f"\n{db_label.replace(chr(10), ' ')}\n")
        f.write("-" * 80 + "\n")
        f.write(f"{'Term':<55} {'FDR':>8}   Genes\n")
        f.write(f"{'-'*55}  {'-'*8}   {'-'*30}\n")
        for _, row in grp.iterrows():
            genes_hit = row.get("Genes", "")
            gene_list_str = ";".join(sorted(str(genes_hit).split(";"))) if genes_hit else "—"
            fdr = row["Adjusted P-value"]
            term = row["term_label"][:54]
            f.write(f"{term:<55} {fdr:>8.4f}   {gene_list_str}\n")
